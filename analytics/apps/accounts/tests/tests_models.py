...
class TestUserAddress(TestCase):
    ...
    def test_create_delete_address(self):
        ...
        address_data = {'user': self.user, 'country': 'Iran', 'city': 'Ahwaz', 'state': 'Khoozestan', 'line': 'Bahonar'}
        # Create Address
        ...
        self.assertIsNone(Address.objects.filter(country='Iran').first())

    def test_create_address_with_user(self):
        """Test if user can create address indirectly"""
        address_1 = self.user.user_address.create(country='Iran', city='Ahwaz')
        address_2 = self.user.user_address.create(line='Valfajr 6', state='Tehran')

        self.assertIn(address_2, Address.objects.all())
        self.assertEqual(address_1.country, self.user.user_address.filter(city='Ahwaz').last().country)
        self.assertEqual(address_1.user.username, self.user.username)
